...
const getAirtableBase = () => {
    ...
    const apiKey = functions.config().airtable?.api_key || process.env.AIRTABLE_API_KEY;
    const baseId = functions.config().airtable?.base_id || process.env.AIRTABLE_BASE_ID;
 ...
 */
async function syncToAirtable(tableName: string, firebaseId: string, data: any) {
 ...
 */
export const syncUsersToAirtable = functions.firestore
 ...
 */
export const syncDocumentsToAirtable = functions.firestore
 ...
 */
export const scheduledFullSyncToAirtable = functions
 ...
 */
export const triggerFullSyncToAirtable = functions
    .runWith({ timeoutSeconds: 540, memory: '512MB' })
